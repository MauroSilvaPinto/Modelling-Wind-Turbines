...
features_index=[0,3,4,5,6,7,8,9,10,12,14,15,16,17,18,19,20]
...
print("Testing turbine "+turbine_names[turbine_tested])

# selecting training and validation data from all turbines except the tested one
# only from 2012-2014
training_indexes=[]
validation_indexes=[]
testing_indexes=[]
...
if turbine_tested==0:
    x_train_merged = x_train[1]
    start=2
    index_used=1
else:
    x_train_merged = x_train[0]
    start=1
    
for i in range(start,len(x_train)):
    if i != turbine_tested:
        x_train_merged=np.concatenate((x_train_merged,x_train[i]), axis=0)



if turbine_tested==0:
    x_validation_merged = x_validation[1]
    start=2
else:
    x_validation_merged = x_validation[0]
    start=1
    
for i in range(start,len(x_train)):
    if i != turbine_tested:
        x_validation_merged=np.concatenate((x_validation_merged,x_validation[i]), axis=0)
        


if turbine_tested==0:
    x_test_merged = x_test[1]
    start=2
else:
    x_test_merged = x_test[0]
    start=1
    
for i in range(start,len(x_train)):
    if i != turbine_tested:
        x_test_merged=np.concatenate((x_test_merged,x_test[i]), axis=0)

  
        
  
if turbine_tested==0:
    y_train_merged = y_train[1]
    start=2
else:
    y_train_merged = y_train[0]
    start=1
        
for i in range(start,len(y_train)):
    if i!=turbine_tested:
        y_train_merged=np.concatenate((y_train_merged,y_train[i]), axis=0)
        
 

if turbine_tested==0:
    y_validation_merged = y_validation[1]
    start=2
else:
    y_validation_merged = y_validation[0]
    start=1
           
for i in range(start,len(y_train)):
    if i!=turbine_tested:
        y_validation_merged=np.concatenate((y_validation_merged,y_validation[i]), axis=0)
        


if turbine_tested==0:
    y_test_merged = y_test[1]
    start=2
else:
    y_test_merged = y_test[0]
    start=1
           
for i in range(start,len(y_train)):
    if i!=turbine_tested:
        y_test_merged=np.concatenate((y_test_merged,y_test[i]), axis=0)
...
for i in range(0,len(x_train)):
    ...
    if i==turbine_tested:
        print("   Tested Turbine " + turbine_names[i]+":")
    else:
...
for i, ax in enumerate(axs):
    ...
    if i==turbine_tested:
        ax.set_ylabel("Tested Turbine " + turbine_names[i] + "\n RMSE:"+str(np.round(rmse,2))+
...
for i in range(0,len(x_test)):
    ...
    if i==turbine_tested:
        ax1.set_title("Tested \n Turbine " + turbine_names[i] + "Pred/Real differences")
    else:
...
for i in range(0,len(x_test)):    
    ...
    if i==turbine_tested:
        print("######### Tested Turbine " + turbine_names[i] + " Pred/Real differences ########")
    else:
        print("######### Training Turbine " + turbine_names[i] + " Pred/Real differences ########")
